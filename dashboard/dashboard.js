...
const programSelectGroup =
  document.getElementById(
    'programSelectGroup'
  );
const programSelect =
  document.getElementById('programSelect');
const workoutLoggingForm =
  document.getElementById(
    'workoutLoggingForm'
  );
...
const noHistoryMsg =
  document.getElementById('noHistoryMsg');
const cardioLoggingForm =
  document.getElementById(
    'cardioLoggingForm'
  );
const dietLoggingForm =
  document.getElementById(
    'dietLoggingForm'
  );
...
const restartTrialBtn =
  document.getElementById(
    'restartTrialBtn'
  );
const smallUpgradeBtn =
  document.getElementById('smallUpgradeBtn');
const saveWorkoutBtn =
  document.getElementById('saveWorkoutBtn');
const logoElement =
  document.getElementById('logoElement');
const coachContactWrapper =
  document.getElementById(
    'coachContactWrapper'
  );
const contactCoachBtn =
  document.getElementById('contactCoachBtn');
const coachContactCard =
  document.getElementById('coachContactCard');
const coachCardName =
  document.getElementById('coachCardName');
const coachCardEmail =
  document.getElementById('coachCardEmail');
const coachCardPhone =
...
async function initDashboard() {
  ...
  const { data: profile, error: pErr } =
    await supabase
      .from('profiles')
      .select(
        'role, current_program_id, ' +
        'trial_ends_at, ' +
        'subscription_status, ' +
        'coach_id, client_status'
      )
  ...
  if (!pErr && profile) {
    ...
    if (profile.coach_id) {
      ...
      if (!cErr && coach) {
        ...
        const coachTrialEnds =
          new Date(coach.trial_ends_at);
        const now = new Date();
        const isCoachExpired =
        ...
        if (isCoachExpired) {
          ...
          if (trialExpirationBanner) {
            ...
            trialExpirationBanner
              .querySelector('h4')
              .textContent =
              "Coaching Group Inactive";
            trialExpirationBanner
              .querySelector('p')
              .textContent =
              "Your coach's account is " +
              "currently inactive. " +
              "Logging is temporarily " +
              "restricted.";
          }
          if (smallUpgradeBtn) {
            smallUpgradeBtn.classList
              .add('hidden');
          }
          if (restartTrialBtn) {
            restartTrialBtn.classList
              .add('hidden');
          }
          lockLoggingInputs(
            'Coaching Account Suspended'
          );
        } else if (profile.client_status ===
                   'suspended' ||
          ...
          if (trialExpirationBanner) {
            ...
            trialExpirationBanner
              .querySelector('p')
              .textContent =
              "Your coach has suspended " +
              "your logging privileges. " +
      ...
      if (isPaid) {
        ...
        if (trialExpirationBanner) {
          trialExpirationBanner.classList
            .add('hidden');
        }
...
function lockLoggingInputs(buttonMessage) {
  if (saveWorkoutBtn) {
    ...
    saveWorkoutBtn.textContent =
      buttonMessage;
  }
  if (cardioLoggingForm) {
    const cardioSubmit =
      cardioLoggingForm.querySelector(
        'button[type="submit"]'
      );
    if (cardioSubmit) {
      cardioSubmit.disabled = true;
      cardioSubmit.style.opacity = '0.5';
      cardioSubmit.textContent =
        'Cardio Locked';
    }
  }
  if (dietLoggingForm) {
    const dietSubmit =
      dietLoggingForm.querySelector(
        'button[type="submit"]'
      );
    if (dietSubmit) {
      dietSubmit.disabled = true;
      dietSubmit.style.opacity = '0.5';
      dietSubmit.textContent =
        'Diet Logging Locked';
    }
  }
}
function applyCoachBranding(coach) {
  ...
  if (coachContactWrapper) {
    coachContactWrapper.classList
      .remove('hidden');
    if (coachCardName) {
      coachCardName.textContent =
        coach.full_name || 'Your Coach';
    }
    if (coachCardEmail) {
      coachCardEmail.textContent =
        coach.email || 'N/A';
    }
    if (coachCardPhone) {
      coachCardPhone.textContent =
        coach.contact_phone || 'N/A';
    }
    if (coachCardAddress) {
      coachCardAddress.textContent =
        coach.contact_address ||
        'Virtual coaching';
    }
  }
}
function setupContactCardListeners() {
  if (!contactCoachBtn ||
      !coachContactCard) return;
  contactCoachBtn.addEventListener(
    'click',
    (e) => {
      e.stopPropagation();
      coachContactCard.classList
        .toggle('hidden');
    }
  );
  ...
  coachContactCard.addEventListener(
    'click',
    (e) => e.stopPropagation()
  );
}
...
if (routineSelect) {
  routineSelect.addEventListener(
    'change',
    async (e) => {
...
function generateExerciseForm(selectedDay) {
  ...
  if (exerciseContainer) {
    ...
    exerciseList.forEach(
      (exerciseName, exIndex) => {
        const exerciseWrapper =
          document.createElement('div');
        exerciseWrapper.className =
          'exercise-log-block';
        exerciseWrapper.setAttribute(
          'data-exercise-name',
          exerciseName
        );
...
if (exerciseContainer) {
  exerciseContainer.addEventListener(
    'click',
    (e) => {
...
if (workoutLoggingForm) {
  workoutLoggingForm.addEventListener(
    'submit',
    async (e) => {
      e.preventDefault();
      if (isTrialExpired) {
      ...
      showStatus("", "");
      const selectedDay = programSelect
        ? programSelect.value
        : ''; 
      const blocks =
...
if (cardioLoggingForm) {
  cardioLoggingForm.addEventListener(
    'submit',
    async (e) => {
      e.preventDefault();
      if (isTrialExpired) {
...
if (dietLoggingForm) {
  dietLoggingForm.addEventListener(
    'submit',
    async (e) => {
      e.preventDefault();
      if (isTrialExpired) {
      ...
      try {
        const selectedDietInput =
          document.querySelector(
            'input[name="dietRating"]' +
            ':checked'
          );
        const activeDropdown =
          document.getElementById(
    ...
    .on(
      ...
      (payload) => {
        const commentFeed =
          document.getElementById(
            `commentsList-` +
            `${payload.new.workout_id}`
          );
        if (commentFeed) {
          appendSingleCommentToFeed(
            commentFeed,
            payload.new
          );
        }
...
function appendSingleCommentToFeed(
  container,
  comment
) {
  const isCoach =
...
async function fetchAndRenderHistory(
  ...
  if (historyGrid) {
    ...
    if (selectedDayFilter &&
      ...
      sortedDates = sortedDates.filter(
        dateKey => {
          ...
          if (matchingLifts.length >
              0) {
            masterDayGroup.lifts =
              matchingLifts;
            return true;
          }
          return false;
    ...
    if (workoutIdsOnScreen.length >
        0) {
      const { data: dbComments } =
        await supabase
          .from('comments')
          .select('*')
          .in(
    ...
    latestDates.forEach(dateStr => {
      ...
      const dietVal =
        dayGroup.diet?.metrics
          ?.diet_rating || null;
      const liftCount =
        dayGroup.lifts.length;
      const cardioLogged =
        dayGroup.cardio ? "🏃 Cardio" : "";
      const headerHTML = `
      ...
      if (liftCount > 0) {
        ...
        Object.keys(exercisesOnThisDay)
          .forEach(exerciseName => {
            const allSetsForThisExercise =
              exercisesOnThisDay[
                exerciseName
              ];
            let topLiftingSet =
              allSetsForThisExercise
                .reduce((max, cur) => {
                  if (!max) return cur;
                  if (cur.weight >
                      max.weight) {
...
const biometricHistoryList =
  document.getElementById(
    'biometricHistoryList'
  );
if (biometricForm) {
  biometricForm.addEventListener(
    'submit',
    async (e) => {
      e.preventDefault();
      if (isTrialExpired) {
      ...
      document.getElementById('resBMR')
        .textContent =
        `${Math.round(bmr)} kcal`;
      document.getElementById('resTDEE')
        .textContent =
        `${Math.round(tdee)} kcal`;
      document.getElementById('resWHR')
        .textContent = whr.toFixed(2);
      const riskContainer =
        document.getElementById(
          'resRisk'
        );
...
async function fetchAndRenderBiometricHistory() {
  ...
  records.forEach(rec => {
    ...
    `;
    biometricHistoryList.appendChild(
      logItem
    );
  });
}
...
async function renderAnalyticsChart() {
  ...
  const timeframeDays = timeframeSelect
  ...
  if (activeChartType === 'body') {
    ...
    if (error || !records ||
      ...
      drawEmptyChartPlaceholder(
        ctx,
        `No biometric history in ` +
        `the last ${timeframeDays} ` +
        `days.`
      );
      return;
    }
    ...
    const weightData = records.map(
      r => r.metrics?.weight || 0
    );
    const waistData = records.map(
      r => r.metrics?.waist || 0
    );
    const bmiData = records.map(
      r => r.metrics?.bmi || 0
    );
    analyticsChartInstance = new Chart(
      ctx,
      {
        type: 'line',
        data: {
    ...
    if (error || !logs ||
      ...
      drawEmptyChartPlaceholder(
        ctx,
        `No training volume logs ` +
        `in the last ${timeframeDays} ` +
        `days.`
      );
      return;
    }
...
function drawEmptyChartPlaceholder(
  ctx,
  message
) {
  const canvas = ctx;
...
function showStatus(text, type) {
  ...
  statusMsg.style.backgroundColor =
    type === "success"
      ? "rgba(57, 255, 20, 0.1)"
      : "rgba(239, 68, 68, 0.1)";
