  ...
  if (!profileError && profile) {
    ...
    if (profile.coach_id) {
      ...
      if (!coachError && coach) {
        ...
        if (isCoachExpired) {
          // Coach expired -> Cascading lockout of the client
          isTrialExpired = true;
          ...
          restartTrialBtn.classList.add('hidden'); // Clients don't pay the app
          lockLoggingInputs('Coaching Account Suspended');
        } else if (profile.client_status === 'suspended' || profile.client_status === 'closed') {
          // Coach is active, but suspended this specific client
          isTrialExpired = true;
          trialExpirationBanner.classList.remove('hidden');
